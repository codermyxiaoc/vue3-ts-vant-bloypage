    ...
    <div class="function">
      <van-row>
        ...
        <van-col span="20">
          ...
          <span class="reply_by"> <!-- @click="gocomment" -->
            <!-- {{ reqlength }}回复
            <van-icon name="arrow" /> -->
          </span>
          <span class="allfunc">
...
<script lang="ts">
import { defineComponent , getCurrentInstance , ref , computed, PropType ,reactive,onMounted } from 'vue'
import { Dialog, Notify } from 'vant'
import { delcom } from '../../api/atr'
  ...
  setup (props) {
    ...
    const deleteart = () =>{
      Dialog.confirm({
        ...
        .then(async () => {
          const result = await delcom(props.artitem.replyuser_id,localStorage.getItem('token') as string)
          console.log(result);
